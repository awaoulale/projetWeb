...
<h4> Rechercher le numero et le nom du document que vous voulez supprimer dans la liste en dessous </h4>
	<form action="supp_doc.php" method="post" enctype="multipart/form-data"> 
	
		  <label> Entrez le nom du document : </label>
	 <input type="text" name="nom_docu"/><br/>
	 
		  <label> Entrez le numero du document : </label>
	 <input type="number" name="id_docu"/><br/>
	 <input type="submit" value="Supprimez votre document"/>
	</form>
...
else
{
	...
	echo '       <label>Supprimer un document</label><br />';
	
		$resultat1=mysqli_query($connect, "SELECT D.id_doc,D.chemin,D.publication,D.statut,D.lieu
		  FROM documents D JOIN utilisateurs U ON U.id=D.id_auteur
		  WHERE U.pseudo='$pseudo' AND U.mail='$mail'");
		...
		if($resultat1)
		{
			while($data=mysqli_fetch_array($resultat1))
			{
				...
				$publication = $data["publication"];
						...
						echo"Numero du document : ".$data['id_doc']."   "."Nom : ".$data['chemin']."   "."Date : ".$data['publication'];
